...
from ..decoders.mask2former_components.pixel_decoder.ops.modules import MSDeformAttn
from .vit_adapter_components.adapter_modules import InteractionBlock, InteractionBlockWithCls, SpatialPriorModule, deform_inputs
...
class ViTAdapter(BaseEncoder):
    ...
    def __init__(self, config: EncoderConfig):
        ...
        if not self.use_cls:
            self.encoder.cls_token = None
        self.num_block = len(self.encoder.blocks)
        # self.pretrain_size = (config.pretrain_size, config.pretrain_size)
        self.interaction_indexes = config.interaction_indexes
        self.add_vit_feature = config.add_vit_feature
        embed_dim = self.encoder.embed_dim

        block_fn = InteractionBlockWithCls if self.use_cls else InteractionBlock
        norm_layer = partial(nn.LayerNorm, eps=1e-6)
        self.level_embed = nn.Parameter(torch.zeros(3, embed_dim))
        self.spm = SpatialPriorModule(inplanes=config.conv_inplane, embed_dim=embed_dim, with_cp=False)
        self.interactions = nn.Sequential(
            *[
                block_fn(
                    dim=embed_dim,
                    num_heads=config.deform_num_heads,
                    n_points=config.n_points,
                    init_values=config.init_values,
                    drop_path=config.drop_path_rate,
                    norm_layer=norm_layer,
                    with_cffn=config.with_cffn,
                    cffn_ratio=config.cffn_ratio,
                    # deform_ratio=config.deform_ratio,
                    extra_extractor=((True if i == len(config.interaction_indexes) - 1 else False) and config.use_extra_extractor),
                    with_cp=config.with_cp,
                )
                for i in range(len(config.interaction_indexes))
            ]
        )
    ...
    #     )
    #     return pos_embed
    def _get_encoder_pos_embed(self, x, w, h):
        previous_dtype = x.dtype
        # npatch = x.shape[1] - 1
        npatch = x.shape[1]
        N = self.encoder.pos_embed.shape[1] - 1
        if npatch == N and w == h:
            return self.encoder.pos_embed
        pos_embed = self.encoder.pos_embed.float()
        ...
        assert N == M * M
        kwargs = {}
        if self.encoder.interpolate_offset:
            # Historical kludge: add a small number to avoid floating point error in the interpolation, see https://github.com/facebookresearch/dino/issues/8
            # Note: still needed for backward-compatibility, the underlying operators are using both output size and scale factors
            sx = float(w0 + self.encoder.interpolate_offset) / M
            sy = float(h0 + self.encoder.interpolate_offset) / M
        ...
        return class_pos_embed.unsqueeze(0), patch_pos_embed
    ...
    def forward(self, x):
        ...
        cls_pos_embed, pos_embed = self._get_encoder_pos_embed(x, h, w)
        ...
        if self.use_cls:
            ...
            pos_embed = torch.cat((cls_pos_embed, pos_embed), dim=1).to(self.encoder.pos_embed.dtype)
